...
class YoutubeDL:
    def __init__(self):
        ...
        self.UNABLE_TO_EXTRACT = ": YouTube said: Unable to extract video data"
        self.OTHER_ERRORS = ["Did not get any data blocks", "HTTP Error 404: Not Found"]
    ...
    def check_and_log_download_output(self, downloader_output):
        ...
        if downloader_output.stderr:
            ...
            if self.UNABLE_TO_EXTRACT in formatted_error:
            ...
            if any(error in formatted_error for error in self.OTHER_ERRORS):
                remove_video_flag = True
                check = True
