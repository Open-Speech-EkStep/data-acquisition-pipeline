...
from selenium_youtube_crawler.gcs import set_gcs_credentials, upload_blob, download_blob, \
    check_blob
...
class Gcs(TestCase):
    def test_set_gcs_credentials(self):
        ...
        os.system("touch " + cred_file_name)
        cred = {"Credentials": {"name": "hello"}}
        with open(cred_file_name, 'w') as cred_file:
            json.dump(cred, cred_file, indent=4)

        set_gcs_credentials(cred_file_name)
    ...
    @patch('selenium_youtube_crawler.gcs.storage.Client')
    ...
    @patch('selenium_youtube_crawler.gcs.storage.Client')
    @patch('selenium_youtube_crawler.gcs.storage.Blob.exists')
